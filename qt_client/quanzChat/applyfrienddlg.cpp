...
ApplyFriendDlg::ApplyFriendDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ApplyFriendDlg),_label_point(2,6)
{
    ui->setupUi(this);

    setWindowFlags(windowFlags() | Qt::FramelessWindowHint);
    this->setObjectName("ApplyFriendDlg");
    ui->name_ed->setPlaceholderText(tr("Nex"));
    ...
    connect(ui->more_lb, &ClickedOnceLabel::clicked, this, &ApplyFriendDlg::ShowMoreLabel);
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lb_ed, &CustomizeEdit::returnPressed, this, &ApplyFriendDlg::SlotLabelEnter);
    connect(ui->lb_ed, &CustomizeEdit::textChanged, this, &ApplyFriendDlg::SlotLabelTextChange);
    connect(ui->lb_ed, &CustomizeEdit::editingFinished, this, &ApplyFriendDlg::SlotLabelEditFinished);
    connect(ui->tip_lb, &ClickedOnceLabel::clicked, this, &ApplyFriendDlg::SlotAddFirendLabelByClickTip);
    ...
    //连接确认和取消按钮的槽函数
    connect(ui->cancel_btn, &QPushButton::clicked, this, &ApplyFriendDlg::SlotApplyCancel);
    connect(ui->sure_btn, &QPushButton::clicked, this, &ApplyFriendDlg::SlotApplySure);
...
void ApplyFriendDlg::InitTipLbs()
{
    int lines = 1;
    for(size_t i = 0; i < _tip_data.size(); i++){
        ...
        connect(lb, &ClickedLabel::clicked, this, &ApplyFriendDlg::SlotChangeFriendLabelByTip);

        QFontMetrics fontMetrics(lb->font()); // 获取QLabel控件的字体信息
        int textWidth = fontMetrics.horizontalAdvance (lb->text())+1; // 获取文本的宽度
...
void ApplyFriendDlg::AddTipLbs(ClickedLabel* lb, QPoint cur_point, QPoint& next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
void ApplyFriendDlg::SetSearchInfo(std::shared_ptr<SearchInfo> si)
{
    _si = si;
...
void ApplyFriendDlg::ShowMoreLabel()
{
    ...
    for(auto & added_key : _add_label_keys){
        ...
        textWidth = fontMetrics.horizontalAdvance (added_lb->text())+1; // 获取文本的宽度
...
void ApplyFriendDlg::addLabel(QString name)
{
    ...
    connect(tmplabel, &FriendLabel::sig_close, this, &ApplyFriendDlg::SlotRemoveFriendLabel);
...
void ApplyFriendDlg::SlotLabelEnter()
{
...
void ApplyFriendDlg::SlotRemoveFriendLabel(QString name)
{
...
//点击标已有签添加或删除新联系人的标签
void ApplyFriendDlg::SlotChangeFriendLabelByTip(QString lbtext, ClickLbState state)
{
...
void ApplyFriendDlg::SlotLabelTextChange(const QString& text)
{
    if (text.isEmpty()) {
        ui->tip_lb->setText("");
        ui->input_tip_wid->hide();
...
void ApplyFriendDlg::SlotLabelEditFinished()
{
    ui->input_tip_wid->hide();
}

void ApplyFriendDlg::SlotAddFirendLabelByClickTip(QString text)
{
...
void ApplyFriendDlg::SlotApplySure()
{
...
void ApplyFriendDlg::SlotApplyCancel()
{
